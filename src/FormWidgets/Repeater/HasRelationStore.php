<?php namespace Larajax\Ui\FormWidgets\Repeater;
...
trait HasRelationStore
{
    ...
    protected function processRelationMode()
    {
        ...
        if ($model && app('model.inspector')->hasRelation($model, $attribute)) {
    ...
    protected function getLoadValueFromRelation()
    {
        ...
        // Deferred bindings are a Rain concept; plain Eloquent reads directly
        $records = method_exists($relationObject, 'withDeferred')
            ? $relationObject->withDeferred($this->getSessionKey())->get()
            : $relationObject->get();
        ...
        // Store the results locally on the model to make it available
        // to nested extensions working with the loaded relation
        if ($records) {
    ...
    {
        $fromModel = $this->getModelFromIndex($fromIndex);

        $model = method_exists($fromModel, 'replicateWithRelations')
            ? $fromModel->replicateWithRelations()
            : $fromModel->replicate();
    ...
    protected function deleteRelationAtIndex($index)
    {
        ...
        // Rain defers the removal against the session key; Illuminate deletes now
        if (method_exists($relationObject, 'remove')) {
            $relationObject->remove($model, $this->getSessionKey());
        }
        else {
            $model->delete();
        }
    ...
    /**
     * addRelationModel binds a model to the relationship, deferred when supported
     */
    protected function addRelationModel($model)
    {
        $relationObject = $this->getRelationObject();

        if (method_exists($relationObject, 'add')) {
            $relationObject->add($model, $this->getSessionKey());
        }
        else {
    ...
    protected function processSaveForRelation($value)
    {
        ...
        foreach ($value as $index => $data) {
            ...
            if (app('model.inspector')->isInstanceOf($model, \October\Contracts\Database\SortableInterface::class)) {
