...
jax.registerControl('input-hotkey', class extends jax.ControlBase {
    static keyMap = {
        esc: 27,
        tab: 9,
        space: 32,
    ...
    connect() {
        ...
        jax.Events.on(this.$target, 'keydown', this.proxy(this.onKeyDown));
    }

    disconnect() {
        this.keyConditions = null;

        jax.Events.off(this.$target, 'keydown', this.proxy(this.onKeyDown));
